...
class __TwigTemplate_f084dc23370293fd7e7d6429974d9275 extends Template
{
...
{
    id: \"{{ token }}\",
    left: {{ \x27%F\x27|format(events.__section__.origin - origin) }},
...
{% if \x27__section__\x27 != name %}
{
    ...
    origin: {{ \x27%F\x27|format(event.origin) }},
    starttime: {{ \x27%F\x27|format(event.starttime) }},
    endtime: {{ \x27%F\x27|format(event.endtime) }},
    duration: {{ \x27%F\x27|format(event.duration) }},
    memory: {{ \x27%.1F\x27|format(event.memory / 1024 / 1024) }},
    elements: {},
    periods: [
        {%- for period in event.periods -%}
        {
            start: {{ \x27%F\x27|format(period.starttime) }},
            end: {{ \x27%F\x27|format(period.endtime) }},
            duration: {{ \x27%F\x27|format(period.duration) }},
            elements: {}
        },
        {%- endfor -%}
    ],
...
{% endmacro %}
", "@WebProfiler/Collector/time.html.twig", "/home/brychepaul/Workspace/intervetion-api/intervention-api/vendor/symfony/web-profiler-bundle/Resources/views/Collector/time.html.twig");
    }
}
